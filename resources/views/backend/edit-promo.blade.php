                        ...
                        <form role="form" class="text-start" method="POST" action="{{ route('admin.promos.edit') }}">
                            ...
                            <div class="input-group input-group-outline my-3">
                                <input type="text" class="form-control" name="name" required
                                    value="{{ $promo->name }}" placeholder="Name of promo">
                                @error('name')
                            ...
                            <div class="input-group input-group-outline my-3">
                                <div>
                                    <input class="form-control" name="startdate" type="date"
                                        value="{{ explode(' ', $promo->start_at)[0] }}" />
                                    @error('startdate')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    <input class="form-control timepicker" name="starttime" type="text"
                                        value="{{ explode(' ', $promo->start_at)[1] }}" />
                                    @error('starttime')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="input-group input-group-outline my-3">
                                ...
                                <div style="width: 10px;"></div>
                                <div>
                                    <input class="form-control timepicker2" name="endtime" type="text"
                                        value="{{ explode(' ', $promo->end_at)[1] }}" />
                                    @error('endtime')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="input-group input-group-outline my-3">
                                <input type="number" class="form-control" name="discount" required placeholder="Discount" value="{{$promo->discount}}">
                                @error('discount')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="input-group input-group-outline my-3">
                                <select name="discount_type" class="form-control"  autocomplete="off">
                                    <option value="0">Select discount type</option>
                                    <option @if ($promo->discount_type=='percent') selected="true" @endif value="percent">Percentage</option>
                                    <option @if ($promo->discount_type=='fixed') selected="true" @endif value="fixed">Fixed amount</option>
                                </select>
                                @error('name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                ...
                                <div class="menu">
                                    ...
                                    <ul>
                                        @foreach ($products as $product)
                                            @if (in_array($product->id, $productIdArr))
                                                <li class="selected" data-value="{{ $product->id }}">
                                                    {{ $product->name }} </li>
                                            @else
...
@section('script')
    <script>
        ...
        $('.timepicker').timepicker({
            ...
            minTime: '0',
            maxTime: '23',
            defaultTime: '{{ explode(' ', $promo->start_at)[1] }}',
            startTime: '00:00',
            dynamic: false,
            dropdown: true,
            scrollbar: true
        ...
        $('.timepicker2').timepicker({
            ...
            minTime: '0',
            maxTime: '23',
            defaultTime: '{{ explode(' ', $promo->end_at)[1] }}',
            startTime: '00:00',
            dynamic: false,
            dropdown: true,
            scrollbar: true
        ...
        items.forEach(li => li.onclick = () => {
            ...
            selected = selected.includes(text) ?
                selected.filter(t => t !== text) : [...selected, text];

            selectedValue = selectedValue.includes(val) ?
                selectedValue.filter(t => t !== val) : [...selectedValue, val];
